...
def update_od_matrix(
    ...
    Parameters
    ----------
    temp_flow_matrix: origin, destination, path, flow, fuel,time, toll
...
def itter_path(
    ...
    conn.execute(
        """
    CREATE OR REPLACE TABLE temp_flow_matrix AS
    SELECT
        o.origin,
        o.destination,
        o.e_id,
...
def network_flow_model(
    ...
    # starts
    total_cost = cost_time = cost_fuel = cost_toll = 0.0
    initial_sumod = remain_od["Car21"].sum()
    assigned_sumod = 0.0
    iter_flag = 1

    # decide whether to create new DB or open existing (resume mode)
    resume_after_itter_path = False
    db_exists = os.path.exists(db_path)
    if db_exists:
        logging.info(
            f"Opening existing DB at {db_path}. Checking for resume candidates..."
        )
        conn = duckdb.connect(db_path)

        # helper to test table existence in main schema
        ...
        has_temp = table_exists(conn, "temp_flow_matrix")
        has_od_results = table_exists(conn, "od_results_iter")
        has_edge_flows = table_exists(conn, "edge_flows")

        if has_temp:
            logging.info(
                "Found existing temp_flow_matrix in DB -> resuming after itter_path."
            )
            resume_after_itter_path = True
        elif has_od_results and has_edge_flows:
            # We don't have temp_flow_matrix but we can rebuild it from od_results_iter + edge_flows
            logging.info(
                "temp_flow_matrix not found, but od_results_iter and edge_flows are present. "
                "Reconstructing temp_flow_matrix now..."
            )
            ...
            logging.info(
                "Reconstructed temp_flow_matrix from od_results_iter and edge_flows."
            )
            resume_after_itter_path = True
        else:
            logging.info(
                "No temp_flow_matrix and insufficient DB state to reconstruct it. "
                "Proceeding to run path-finding/itter_path as normal."
            )
    else:
        # create new DB
        logging.info(f"Creating new DB at {db_path}.")
        if os.path.exists(db_path):
            os.remove(db_path)
        conn = duckdb.connect(db_path)

    # ensure the persistent tables odpfc / isolated_od exist
    conn.execute(
        """
        CREATE TABLE IF NOT EXISTS isolated_od (
            origin_node VARCHAR,
    ...
    conn.execute(
        """
        CREATE TABLE IF NOT EXISTS odpfc (
            origin VARCHAR,
            destination VARCHAR,
            path VARCHAR,
            flow DOUBLE,
    ...
    while total_remain > 0:
        ...
        temp_isolation = (
            isolated_flow_matrix["flow"].sum() if len(isolated_flow_matrix) else 0.0
        )
        logging.info(f"Initial isolated flows: {temp_isolation}")

        # if we are resuming and temp_flow_matrix already exists, skip pathfinding and itter_path
        if resume_after_itter_path and iter_flag == 1:
            logging.info("Resuming after itter_path: using temp_flow_matrix from DB.")
            # load temp_flow_matrix into pandas for downstream checks if desired (we mostly use DB queries)
            # temp_flow_matrix = conn.execute("SELECT * FROM temp_flow_matrix").fetchdf()
        else:
            ...
            logging.info(
                f"The least-cost path flow allocation time: {time.time() - st}."
            )
            temp_flow_matrix = (
                pd.DataFrame(
                    list_of_spath,
            ...
            # calculate allocated and isolated flows
            (
                temp_flow_matrix,
                isolated_flow_matrix,
                temp_isolation,
            ...
            logging.info(
                "Create temp_flow_matrix table in duckdb by calling itter_path..."
            )
            itter_path(
                network,
                road_links,
            ...
            # after successful itter_path we can set resume_after_itter_path False (normal flow)
            resume_after_itter_path = False

        # --- post-itter_path analysis (works whether we just ran itter_path or resumed) ---
        assigned_iter_sum = (
            conn.execute(
        ...
        conn.execute(
            """
            INSERT INTO odpfc (origin, destination, path, flow, fuel, time, toll)
            SELECT origin, destination, e_id, flow, fuel, time, toll FROM temp_flow_matrix
            """
        )
        ...
        logging.info("Computing edge flows...")
        temp_edge_flow = conn.execute(
            """
            SELECT e AS e_id, SUM(flow) AS flow
            FROM (
                SELECT flow, UNNEST(e_id) AS e FROM temp_flow_matrix
            ) AS t
            GROUP BY e
            """
        ).fetchdf()
        ...
        # Recalculate edge speeds for edges that changed
        ...
        # update remain od
        conn.register("remain_od_tmp", remain_od)
        conn.execute(
            """
            CREATE OR REPLACE TEMP TABLE remain_od AS
            SELECT origin_node AS origin, destination_node AS destination, Car21 AS Car21
            FROM remain_od_tmp;
            """
        )
        ...
        conn.execute(
            ...
            SELECT
                r.origin,
                r.destination,
                (r.Car21 - COALESCE(a.flow_assigned, 0.0)) AS Car21_raw,
                GREATEST((r.Car21 - COALESCE(a.flow_assigned, 0.0)), 0.0) AS Car21
            FROM remain_od r
            LEFT JOIN (
                SELECT origin, destination, SUM(flow) AS flow_assigned
        ...
        remain_od = conn.execute(
            """
            SELECT origin AS origin_node, destination AS destination_node, Car21
        ...
        # cleanup temp tables created in this iteration
        conn.execute("DROP TABLE IF EXISTS od_results_iter")
        ...
        conn.execute("DROP TABLE IF EXISTS remain_od_updated")

        # checkpoints for stopping
        if percentage_sumod >= 0.99:
            temp_isolation = remain_od["Car21"].sum()
            if temp_isolation:
                conn.append(
                    "isolated_od",
                    remain_od[["origin_node", "destination_node", "Car21"]].rename(
                        columns={"Car21": "flow"}
                    ),
                )
            logging.info(
        ...
        # update network structure for next iteration
