...
macchub_path = Path(
    load_config()["paths"]["MACCHUB"]
)  # ./mistra/MACCHUB/processed_data
tqdm.pandas()
...
def main(
    ...
    with open(macchub_path / "parameters" / "flow_breakpoint_dict.json", "r") as f:
    ...
    ) = load_scenarios(base_path=macchub_path)
