...
def main(
    ...
    for root, _, files in os.walk(
        ...
        for file in files:
            road_links_path = Path(root) / file
            road_links_list.append(road_links_path)
    ...
    # )
    for road_links_path in road_links_list:
        road_links = gpd.read_parquet(road_links_path)
        flood_key = road_links_path.stem.split("_")[-1]
        breakpoint()
        ...
        for day in range(111):
            ...
            od_path_file["disrupted_links"] = od_path_file["disrupted_links"].apply(
                list
            )
            ...
            disrupted_od["flow"] = np.maximum(
                0,
                disrupted_od["flow"]
                - disrupted_od["min_capacities_of_disrupted_links"],
            )

            # Adjust road flows
            road_links["disrupted_flow"] = 0.0
            disrupted_edge_flow = get_flow_on_edges(
                disrupted_od, "e_id", "path", "flow"
            )
