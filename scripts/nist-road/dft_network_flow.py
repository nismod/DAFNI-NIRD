...
def main(
    ...
    db_path = nist_path / "dbs" / f"nist_{od}.duckdb"
    logging.info(f"Database path is: {db_path}")

    # outpath
    out_path = nist_path.parent / "outputs"
    out_path.mkdir(parents=True, exist_ok=True)
    iso_out_path = out_path / f"isolation_{od}.pq"
    odpfc_out_path = out_path / f"odpfc_{od}.pq"
    ...
    # export files
    road_links.to_parquet(out_path / f"edge_flow_{od}.gpq")
...
if __name__ == "__main__":
    ...
    try:
        od, num_of_chunk, num_of_cpu = sys.argv[1:]
        sample_stride = 1
        main(
            od,
            int(num_of_chunk),
        ...
        logging.info(
            ...
            "od (od_agg/od_agg_gt1), num_of_chunk (int), num_of_cpu (int)."
        )
